 ...
 *
 * # 成员:
 *      fil_BlsPrivateKey - 私钥，
 *      mnemonic          - 助记词（字节数组），需要以空格(32)字符切割
 * */
typedef struct {
	fil_BLSPrivateKey private_key;
 ...
 *
 * # 成员:
 *      network  - 网络号，测试网（‘t’-116），主网（'f'-102）
 *      protocol - 算法类型
 *                 1 - secp256k1
 *                 3 - bls
 *      address  - 地址
 ...
 *      prams_len       - 交易携带数据的长度
 *
 * # 返回值: 签名后数据，包括交易cid和签名
 *
 * */
fil_SignedMessageResult *sign_transaction_message(uint8_t typ,
 ...
 *
 * */
void fil_destroy_signed_message(fil_SignedMessageResult* ptr);
 ...
 *
 * # 返回值:
 *      typ - 算法类型, 1-secp256k1, 3-bls
 *      fil_WalletResponse - 钱包指针
 *
 * */
fil_WalletResponse* fil_create_wallet(uint8_t typ);
 ...
 *
 * */
void fil_destroy_wallet(fil_WalletResponse*);
 ...
 *      count - 字符个数
 * */
fil_BLSPrivateKey* fil_recover_private_key(uint8_t typ, const uint8_t* words_ptr, uint8_t count);
 ...
 *
 * */
void fil_destroy_bls_private_key(fil_BLSPrivateKey* ptr);
 ...
 *                 1 - secp256k1
 *                 3 - bls
 *      raw_private_key_ptr - 私钥指针
 *
 * # 示例:
 *      BlsAddress* addr_ptr = fil_private_key_to_address(116, 3, priv_key.inner);
 * */
fil_Address* fil_private_key_to_address(uint8_t network, uint8_t protocol, const uint8_t *raw_private_key_ptr);
/**
 * # 功能: 销毁地址
 *
 * # 参数: 地址指针
 ...
 *
 * */
void fil_destroy_address(fil_Address* ptr);
 ...
 *      ExportResult - 导出结果
 * */
fil_ExportResult* fil_export_private_key(uint8_t typ, const uint8_t *raw_private_key);
 ...
 *
 * */
void fil_destroy_export_result(fil_ExportResult* ptr);
 ...
 *      fil_BLSPrivateKey - 私钥
 * */
fil_BLSPrivateKey* fil_import_private_key(const uint8_t* raw_bytes);
